...
%========================================================================
function [R,dR,Q] = ...
    HB_residual_reduced(X,system,H,N,analysis_type,V,varargin)
...
% Jacobian using AFT
function [F,dF] = ...
    HB_nonlinear_forces_AFT(Q,dQ,Om,dOm,H,N,nonlinear_elements,V)
%% Initialize output
F = zeros(size(Q));
...
%                 end
                for iii = 1 : size( q , 1 )
                    x = q(iii, :)';                    
                    u = V*x;
                    [K_V, F_V] = myAssembly.tangent_stiffness_and_force(u);
                    df_dql_all(iii, :, :) = K_V - K0;
                    fnl(iii, :) = F_V - K0*x;                    
                end
