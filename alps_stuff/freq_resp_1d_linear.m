function [lin_full,lin_red,lin_an]=...
    freq_resp_1d_linear(freq,amplification,BeamAssembly,BeamReducedAssembly,varargin) 
%% variables for IC and return
fft_cycles=2;

lin_full=zeros(length(freq),length(amplification));
lin_red=zeros(length(freq),length(amplification));
lin_an=zeros(length(freq),length(amplification));

%first initial condition full
init_cond_full=zeros(BeamAssembly.Mesh.nDOFs, 1);

TI_lin_full.Solution.q=BeamAssembly.constrain_vector(init_cond_full);
TI_lin_full.Solution.qd=BeamAssembly.constrain_vector(init_cond_full);

%first initial condition red
V_size=size(BeamReducedAssembly.V);
init_cond_red=zeros(V_size(2),1);

TI_lin_red.Solution.q=init_cond_red;
TI_lin_red.Solution.qd=init_cond_red;

%% freq response type

if isempty(varargin)
    isfull=true;
    isreduced=true;
    isanalytical=true;   
else
    varargin=string(varargin);
    if ismember("full",varargin)
        isfull=true;
    else 
        isfull=false;
    end
    if ismember("reduced",varargin)
        isreduced=true;
    else 
        isreduced=false;
    end
    if ismember("analytical",varargin)
        isanalytical=true;
    else 
        isanalytical=false;
    end
    if ismember("none",varargin)
        return
    end
end
    %% some extra variables for analytical sol
    
Mc=BeamAssembly.constrain_matrix(BeamAssembly.DATA.M);
Dc=BeamAssembly.constrain_matrix(BeamAssembly.DATA.C);
Kc=BeamAssembly.constrain_matrix(BeamAssembly.DATA.K);
%% main loop
    for curr_freq_step=1:length(freq)
        for curr_amp_step=1:length(amplification)
        %% External force 

        omega_ext=freq(curr_freq_step)
%         pause(5)
        
        forced_dof = round(BeamAssembly.Mesh.nNodes/2) * 3 - 1; % middle node, vertical dof
        F = zeros(BeamAssembly.Mesh.nDOFs, 1);
        F( forced_dof ) = 1;
        Famp = amplification(curr_amp_step);
        F_ext = @(t) Famp * F * sin(omega_ext * t);
        
        %force for analytical sol
        Fextc = BeamAssembly.constrain_vector( F );
        forced_dof_c = BeamAssembly.free2constrained_index( forced_dof );
        
        % time step for integration
        T =  2*pi/omega_ext; % time period of forcing
        dt = T/50/4;
        tmax = 160*T; 
        Fs=1/dt;
        %% Linear full
        if isfull
        % Initial condition: previous step
        q0 = TI_lin_full.Solution.q(:,end);
        qd0 = TI_lin_full.Solution.qd(:,end);
        qdd0 = BeamAssembly.constrain_vector(init_cond_full);

        % Instantiate object for linear time integration
        TI_lin_full = ImplicitNewmark('timestep',dt,'alpha',0.005,'linear',true);

        % Linear Residual evaluation function handle
        residual_lin = @(q,qd,qdd,t)residual_linear(q,qd,qdd,t,BeamAssembly,F_ext);
        TI_lin_full.Integrate(q0,qd0,qdd0,tmax,residual_lin);

        % obtain full solution
        TI_lin_full.Solution.u = BeamAssembly.unconstrain_vector(TI_lin_full.Solution.q);
        
        last_cycles_sol=TI_lin_full.Solution.u(forced_dof,end-fft_cycles*T/dt:end);
        [~, Y, ~] = fft_yFs(last_cycles_sol, Fs);
        w=max(Y);
        lin_full(curr_freq_step,curr_amp_step)=norm(w);
        end
        %% Linear reduced
        if isreduced
        q0 = TI_lin_red.Solution.q(:,end);
        qd0 = TI_lin_red.Solution.qd(:,end);
        qdd0 = init_cond_red;

        TI_lin_red = ImplicitNewmark('timestep',dt,'alpha',0.005,'linear',true);

        % Modal linear Residual evaluation function handle
        Residual_lin_red = @(q,qd,qdd,t)residual_reduced_linear(q,qd,qdd,t,BeamReducedAssembly,F_ext);

        % time integration
        TI_lin_red.Integrate(q0,qd0,qdd0,tmax,Residual_lin_red);
        TI_lin_red.Solution.u = BeamReducedAssembly.V * TI_lin_red.Solution.q;
        
        last_cycles_sol=TI_lin_red.Solution.u(forced_dof,end-fft_cycles*T/dt:end);
        [~, Y, ~] = fft_yFs(last_cycles_sol, Fs);
        w=max(Y);
        lin_red(curr_freq_step,curr_amp_step)=norm(w);
        end
        
        %% linear analytical
        if isanalytical
        w = omega_ext;
        fr_linear = ...
            (-w^2*Mc + 1i*w*Dc + Kc) \ Fextc * amplification(curr_amp_step);
        lin_an(curr_freq_step,curr_amp_step)=abs(fr_linear(forced_dof_c));
        end
        end
    end


end